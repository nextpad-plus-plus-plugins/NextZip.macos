...
static std::string lastMethod(NextZipEngine& r) {
...
int main(int argc, char** argv) {
	...
	// 1) 7z + LZMA2 + dict/word/solid/threads at level 9
	{
		...
		NextZipEngine::CompressOptions o;
	...
	// 2) 7z + PPMd
	{
		...
		NextZipEngine r; r.setEnginePath(eng); r.open(out + "/p.7z");
	...
	// 3) zip + AES-256 encryption + Deflate
	{
		...
		NextZipEngine r; r.setEnginePath(eng); r.open(out + "/enc.zip");
		...
		NextZipEngine x; x.setEnginePath(eng); x.open(out + "/enc.zip");
		bool ok  = x.extract({}, out + "/ex_ok",  "secret");
		NextZipEngine y; y.setEnginePath(eng); y.open(out + "/enc.zip");
	...
	{
		NextZipEngine e; e.setEnginePath(eng);
		NextZipEngine::CompressOptions o; o.format = "zip"; o.level = 1; o.pathMode = 1;
		if (!e.compress(out + "/full.zip", o, {in})) { printf("FAIL full: %s\n", e.error().c_str()); return 1; }
		NextZipEngine r; r.setEnginePath(eng); r.open(out + "/full.zip");
